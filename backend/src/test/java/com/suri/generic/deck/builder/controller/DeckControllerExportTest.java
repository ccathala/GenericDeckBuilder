...
class DeckControllerExportTest {
    ...
    void exportDeck_withDefaultLocale_shouldSucceed() throws Exception {
        ...
        mockMvc.perform(get("/api/decks/{id}/export", testDeckId)
                ...
                .andExpect(jsonPath("$.exportedContent").value("4 anneau d'azur\n2 épée de lumière\n3 boule de feu"));

        verify(deckExportService).exportDeck(testDeckId, "fr", testUser);
    }
    ...
    void exportDeck_withCustomLocale_shouldSucceed() throws Exception {
        ...
        mockMvc.perform(get("/api/decks/{id}/export", testDeckId)
                ...
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.exportedContent").value("4 azure ring\n2 light sword\n3 fireball"));

        verify(deckExportService).exportDeck(testDeckId, locale, testUser);
    }
    ...
    void exportDeck_withUnauthorizedUser_shouldReturn403() throws Exception {
        // Given
        when(deckExportService.exportDeck(eq(testDeckId), eq("fr"), any(User.class)))
                .thenThrow(new UnauthorizedAccessException("L'utilisateur n'est pas autorisé à exporter ce deck"));
